# 株式会社ハートビーツさんから出された課題「lamp環境の構築」の課程14
激動の数日、様々な選択肢。その先に何を選ぶのか、目まぐるしく変化する状況の中、どこにチャンスを見出すのか。

## WSL+UbuntuのVirtualHostの設定2
昨日の更新を忘れたので、今日こそはVirtualHostの設定を終わらせたい。

'https://www.digitalocean.com/community/tutorials/how-to-install-the-apache-web-server-on-ubuntu-20-04-ja'
前回に引き続き、ここを参考にした。

相変わらず接続出来ず、
"$ sudo a2ensite"の後、"$ sudo a2disssite"をやったかどうかと聞かれた。
前回の資料を見る限り、やっていなかった。

"a2ensite"と"a2disssite"について、一応マニュアルを確認する。
...
DESCRIPTION
       ...
       is being redirected there. Thus it should be called 000-default in order to sort before the remaining hosts to
       be loaded first.
...
OPTIONS
       ...
       -m, --maintmode
              Enables the maintainer mode, that is the program invocation is effectuated  automatically  by  a  main‐
              tainer script. This switch should not be used by end users.
```

"a2disssite"について記述があるけど、一応やってみる。

```
$ man a2disssite
No manual entry for a2disssite
```

virtual host(恐らく*.confファイル)を有効にするか無効にするかの設定が出来るようだ。

とりあえず、無効にする前のファイルを覗いてみる。

```
$ cd /etc/apache2/sites-available
$ ls
000-default.conf  default-ssl.conf  hbtask.conf
...
localhostがなぜ繋がるのか、その理由の一端を見た気がした。
...
localhostでfamilyname-hbtask.localディレクトリのファイルが表示されるようになった。
